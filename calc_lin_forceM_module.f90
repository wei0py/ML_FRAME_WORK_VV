...
!********************covariance
     ...
     do itype=1,ntype
     open(10,file=cov_ref_path//char(itype+48),form="unformatted")
     rewind(10)
...
!cccc  calculate covariance. 
     ...
     do 400 itype=1,ntype
      ...
      feat_cov(1,1,itype),nfeat2m,feat2_type(1,1,itype),nfeat2m,0.d0,S,num_rp)
      ...
      enddo
       ...
       S(i1,i2)=1.d0/(S(i1,i2)+ave_dd_cov(itype)/ave_dive_cov(itype))
       enddo
       enddo

        call dgemm('N','N',num_rp,num_case,num_rp,1.d0, &
       dd_cov(1,1,itype),num_covm,S,num_rp,0.d0,S2,num_rp)
