...
!     & 0.d0,AA_type(1,1,itype),nfeat2m)
     ...
     & 0.d0,AA_type(1,1,itype),num_refm)
       ...
       sum=0.d0
       do i=1,num(itype) ! num of atom
       sum=sum+energy(ind_type(i,itype))*feat_new_type(j,i,itype)
       enddo
       BB_type(j,itype)=sum
       enddo
...
ccccc Now, we have the new features, we need to calculate the distance to reference state
       ...
       call dgemm('N','T',nfeatNtot,nfeatNtot,3*natom,weight_F,
     & VV,nfeatNtot,VV,nfeatNtot,1.d0,AA,nfeatNtot)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       call dgesv(nfeatNtot,1,AA,nfeatNtot,ipiv,BB,
     &     nfeatNtot,info)  
!cccccccccccccccccccccccccccccccccccccccccccccccccc
