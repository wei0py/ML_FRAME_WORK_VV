       program test_VV_force_step1

!  This code calculate just a few steps prediction, and write it out in 
! energyVV.pred.test,forceVV.pred.test,energyVV.pred.tot.test
!  This willl be subsequently checked by runing the small code test_VV_force_step2.f
!  with the corresponding MOVEMENT (which is used to generate the feature) move to the
!  directory, and name MOVEMENT.test
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       write(6,*) "input num of image to test"
       read(5,*) nimage_test
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccc
        do itype=1,ntype
        open(12,file="OUT.VV_index."//char(itype+48))
        rewind(12)
...
!       allocate(BB_type(num_refm,ntype))
       ...
       open(10,file="linear_VV_fitB.ntype")
       rewind(10)
       read(10,*) nfeatNtot_t
...
!ccccccccccccccccccccccccccccccccccccccccccccc
       ...
       open(70,file="energyVV.pred.tot.test")
       rewind(70)
 
c       do itype=1,ntype
       open(20,file="energyVV.pred.test")
       rewind(20)
       open(40,file="forceVV.pred.test")
       rewind(40)

!cccccccccccccccccccccccccccccccccccccccccccccc

c       do 900 sys=1,sys_num
       do 900 sys=1,1
...
ccccccccccccccccccccccccccccccccccccccccccc
     ...
     & 1.d0,PV(1,1,itype),
     & nfeat1m,feat_type(1,1,itype),nfeat1m,0.d0,feat2_type(1,1,itype),
     & nfeat2m)
       enddo
...
ccc dfeat(nfeat1,natom,j_neigh,3): dfeat(j,i,jj,3)= d/dR_i(feat(j,list_neigh(jj,i))
ccccccccccccc
...
ccccccccc Note: num(itype) is rather large, in the scane of natom*num_neigh
       ...
       call dgemm('T','N',nfeat2(itype),num(itype),nfeat1(itype),
     &  1.d0,PV(1,1,itype),
     & nfeat1m,dfeat_type(1,1,itype),nfeat1m,0.d0,
...
ccccccccccccccccccccccccccccccccc
        ...
        dfeat_ext1(i,j,id)=-feat_ext1(i,j,id)*2*(feat2(j,i)-xp1(1,id))
     &                              /xp1(2,id)**2
        enddo
        enddo
        enddo
        ...
        dfeat_ext2(i,j,id)=-feat_ext2(i,j,id)*2*(feat2(j,i)-xp(1,id))
     &                      /xp(2,id)**2
        enddo
        enddo
        enddo
...
ccccc Now, we have the new features, we need to calculate the distance to reference state
       ...
       do i=1,natom
        write(20,*) i,energy(i),energy_pred(i)
        write(40,*) i,force(1,i),force_pred(1,i)
        write(40,*) i,force(2,i),force_pred(2,i)
        write(40,*) i,force(3,i),force_pred(3,i)
       enddo
        write(20,*) "--------------------"
        write(40,*) "--------------------"
