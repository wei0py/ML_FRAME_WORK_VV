...
def GenMOVEMENTLpp(interval,MOVEMENTlppFile):
    trainData=read_data(os.path.join(pm.trainSetDir,'MOVEMENT.xyz'))
    pp.collectAllSourceFiles()
    with open(MOVEMENTlppFile,'w') as lppF:
        count=0
        for system in pm.sourceFileList:
            infodata=np.loadtxt(os.path.join(system,'info.txt'))
            natom=infodata[1]+2
...
if __name__=='__main__':
    ...
    GenMOVEMENTLpp(interval,MOVEMENTlppFile)
