...
class MdImage(Atoms,Image):
    ...
    def fromImage(anImage,calc=calc_lin,isCheckVar=False,isReDistribute=True):
        ...
        Atoms.__init__(self,
        scaled_positions=cp.array(self.pos),
        cell=Cell(cp.array(self.cupyCell)),
        pbc=True,
    ...
    def fromAtomConfig(atomConfigPath,calc=calc_lin,isCheckVar=False,isReDistribute=True):
        ...
        Atoms.__init__(self,
        scaled_positions=cp.array(pos),
        cell=Cell(cp.array(cell)),
        pbc=True,
        numbers=self.atomTypeList)
        ...
        with open(atomConfigFilePath,openMode) as atomConfigFile:
            ...
            if not isApplyConstraint:
                ...
                for i in range(len(self)):
                    ...
                    str(float(sPos[i,2]))+'  '+lineTail+str(energies[i])+'  '+str(forces[i,0])+'  '+str(forces[i,1])+'  '+str(forces[i,2])+'  '+str(self.cov_atom[i])+'\n')
    ...
    def calc_feat(self):
        cell=np.asfortranarray(cp.array(self.cupyCell.T))
...
#TODO:
        ...
        self.cov_atom=np.array(self.calc.cov_atom)
